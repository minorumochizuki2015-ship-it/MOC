...
class OrchDashboard:
    ...
    def __init__(self, config: Optional[OrchDashboardConfig] = None):
        ...
        try:
            self.config.host = os.getenv("ORCH_DASHBOARD_HOST", self.config.host)
            port_env = os.getenv("ORCH_DASHBOARD_PORT")
            if port_env and str(port_env).isdigit():
                self.config.port = int(port_env)
        except Exception:
            pass
        # 短期復旧フラグ（ダミー応答を返す）
        ...
        # 簡易レートリミット用ストア（メモリ内）とレジストリ書き込みロック
        self._rate_state = {}
    ...
    def _load_secrets(self) -> Dict[str, Any]:
        """シークレットの読み込み（例: JWT/HMAC）。存在しなければダミー値。"""
        default = {
            "jwt": {"secret": "CHANGE_ME_DEV_ONLY"},
            "hmac": {"secret": "CHANGE_ME_DEV_ONLY"},
        }
        try:
            p = Path("config/secrets.json")
            if p.exists():
                data = json.loads(p.read_text(encoding="utf-8"))
                return {
                    "jwt": {"secret": data.get("jwt", {}).get("secret", default["jwt"]["secret"])},
                    "hmac": {
                        "secret": data.get("hmac", {}).get("secret", default["hmac"]["secret"])
                    },
                }
            return default
        except Exception:
    ...
    def _check_auth(self, auth_cfg: Dict[str, Any]) -> (bool, str):
        """Authorization(Bearer JWT)またはX-Signature(HMAC)の検証"""
        try:
            ...
            if authz.startswith("Bearer "):
                ...
                try:
                    ...
                    decoded = jwt.decode(
                        token,
                        self.secrets.get("jwt", {}).get("secret", ""),
                        algorithms=["HS256"],
                        options={"verify_aud": False},
                    )
                    iss = auth_cfg.get("issuer")
                    aud = auth_cfg.get("audience")
                    if iss and decoded.get("iss") and decoded.get("iss") != iss:
                        return False, "invalid_issuer"
                    if aud and decoded.get("aud") and decoded.get("aud") != aud:
                        return False, "invalid_audience"
                    return True, ""
                except Exception as e:
                    return False, f"jwt_error:{e}"
            ...
            if sig:
                ...
                secret = self.secrets.get("hmac", {}).get("secret", "")
    ...
    def _before_request(self):
        ...
        request.environ["X-Request-ID"] = cid

        # レートリミット（/api/agents/* のみ）
        try:
            if request.path.startswith("/api/agents/"):
                limit = int(self.api_config.get("security", {}).get("rate_limit_per_min", 120))
                key = f"{request.remote_addr}:{datetime.now().strftime('%Y%m%d%H%M')}"
        ...
        # 認証必須なら検証
        if self.api_config.get("auth", {}).get("required", False) and request.path.startswith(
            "/api/agents/"
        ):
            ok, err = self._check_auth(self.api_config.get("auth", {}))
            if not ok:
                return jsonify({"success": False, "error": "unauthorized", "detail": err}), 401

    def _after_request(self, response):
        try:
            cid = request.environ.get("X-Request-ID")
            self.logger.info(
                json.dumps(
                    {
                        "cid": cid,
    ...
    def _setup_routes(self):
        ...
        # グローバルエラーハンドラ（統一JSONフォーマット）
        @self.app.errorhandler(Exception)
        def handle_global_error(e):
            try:
                self.logger.error(f"Global error: {e}")
                return (
                    jsonify(
                        {
                            "success": False,
        ...
        def realtime_page():
            ...
            try:
                return render_template("realtime_dashboard.html", title="Phase 4 リアルタイム監視")
            except Exception as e:
        ...
        # Debug: list all registered routes for auditing
        ...
        # Minimal /api/system-health endpoint (audit-critical)
        ...
        # 後方互換: アンダースコア版をハイフン版へリダイレクト
        @self.app.route("/api/system_health")
        def api_system_health_underscore():
            return redirect(url_for("api_system_health"))

        # Agents API (minimal implementation to satisfy audit)
        registry_path = os.path.join(os.path.dirname(__file__), "data", "agents_registry.json")
        ...
        def api_agents_list():
            return jsonify(_load_registry()), 200
        ...
        def api_agents_register():
            try:
                ...
                agent_id = (
                    ...
                    or f"agent-{int(datetime.now().timestamp())}"
                )
                entries = _load_registry()
                existing = next((e for e in entries if e.get("id") == agent_id), None)
                record = {
                    "id": agent_id,
                ...
                return jsonify({"success": True, "id": agent_id}), 200
            except Exception as e:
                return jsonify({"success": False, "error": str(e)}), 400

        @self.app.route("/api/agents/heartbeat", methods=["POST"])
        def api_agents_heartbeat():
            try:
                ...
                existing["heartbeat_at"] = datetime.now().isoformat()
                _save_registry(entries)
                return jsonify({"success": True, "id": agent_id}), 200
            except Exception as e:
                return jsonify({"success": False, "error": str(e)}), 400
        ...
        def api_agents_report():
            try:
                ...
                reports.append({"timestamp": datetime.now().isoformat(), "data": payload})
        ...
        def metrics():
            ...
            try:
                ...
                total_agents = Gauge(
                    "orch_agents_total", "Total number of agents", registry=registry
                )
                total_agents.set(len(agents))

                # Status breakdown
                try:
                    from collections import Counter

                    status_counts = Counter([(a.get("status") or "unknown") for a in agents])
                except Exception:
                    status_counts = {}
                agent_status = Gauge(
                ...
                # Latest heartbeat age (seconds) - max over agents
                heartbeat_age = Gauge(
                    "orch_agents_heartbeat_age_max_seconds",
                    "Max seconds since last heartbeat across agents",
                    registry=registry,
                )
                try:
                    now = time.time()
                    ...
                    for a in agents:
                        ...
                        if ts:
                            ...
                            if isinstance(ts, (int, float)):
                                ...
                                try:
                                    # Fallback: parse ISO8601
                                    dt = datetime.fromisoformat(str(ts).replace("Z", ""))
                                    ages.append(max(0, now - dt.timestamp()))
                                except Exception:
                                    pass
                    heartbeat_age.set(max(ages) if ages else 0)
                except Exception:
                    heartbeat_age.set(0)

                # Agents API audit results
                audit_ok = Gauge(
                    "orch_agents_api_audit_ok",
                    "Agents API audit overall pass (1/0)",
                    registry=registry,
                )
                audit_pass_count = Gauge(
                    "orch_agents_api_audit_pass_count",
                    "Agents API audit PASS occurrences",
                    registry=registry,
                )
                audit_last_epoch = Gauge(
                    "orch_agents_api_audit_last_run_epoch",
                    "Agents API audit last run epoch seconds",
                    registry=registry,
                )
                try:
                    audit_path = Path(self.base_dir) / "ORCH" / "REPORTS" / "AGENTS_API_AUDIT.md"
                    ok = 0
                    pcount = 0
                    last_epoch = 0
                    if audit_path.exists():
                        txt = audit_path.read_text(encoding="utf-8", errors="ignore")
                        pcount = txt.count("PASS")
                        # Consider fail-free as OK
                        ok = 1 if ("PASS" in txt and "FAIL" not in txt) else 0
                        last_epoch = int(audit_path.stat().st_mtime)
                    audit_ok.set(ok)
                    audit_pass_count.set(pcount)
                    audit_last_epoch.set(last_epoch)
                except Exception:
                    audit_ok.set(0)
    ...
    def _agents_registry_path(self) -> Path:
        try:
            # Use workspace root (two levels up) as unified agents registry path
            return Path(__file__).resolve().parent.parent / "data" / "agents_registry.json"
        except Exception:
            return Path("data/agents_registry.json")

    def _load_agents(self) -> List[Dict[str, Any]]:
        try:
            p = self._agents_registry_path()
            if not p.exists():
                return []
    ...
    def _save_agents(self, items: List[Dict[str, Any]]) -> None:
        try:
            ...
            self.logger.error(f"Failed to save agents registry: {e}")
    ...
    def _save_instructions(self, items: List[Dict[str, Any]]) -> None:
        ...
        def api_agents_list():
            try:
                ...
                self.logger.error(f"/api/agents/list error: {e}")
                return jsonify({"success": False, "error": str(e)}), 500
            ...
            try:
                ...
                for a in agents:
                    if a.get("id") == agent_id:
                        ...
                        allowed_fields = ["role", "worktree", "capabilities", "status"]
        ...
        def api_agents_remove():
            try:
                ...
                self.logger.error(f"/api/agents/remove error: {e}")
                return jsonify({"success": False, "error": str(e)}), 500
        ...
        def api_agents_report():
            try:
                ...
                try:
                    ...
                    content = (
                        self.audit_md_path.read_text(encoding="utf-8")
                        if self.audit_md_path.exists()
                        else ""
                    )
                    content += "\n" + line
