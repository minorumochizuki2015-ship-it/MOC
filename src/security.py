...
class SecurityManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._enable_cleanup_on_init = config.get("security", {}).get(
            "enable_cleanup_on_init", True
        )
        ...
        # Allowable clock skew (seconds) for iat/exp/nbf validation to satisfy contract tests
        self.jwt_leeway_seconds = config.get("jwt", {}).get("leeway_seconds", 120)
        ...
        self.init_database()
        if self._enable_cleanup_on_init:
    ...
    def _connect(self):
        ...
        # Use check_same_thread=False to allow concurrent verification in tests
        conn = sqlite3.connect(
            self.db_path,
            timeout=float(self.config.get("database", {}).get("timeout_seconds", 5)),
            check_same_thread=False,
        )
        try:
            # Enable WAL to reduce writer blocking readers on Windows
            conn.execute("PRAGMA journal_mode=WAL;")
            # Busy timeout to mitigate transient file locks
            busy_ms = int(
                float(self.config.get("database", {}).get("busy_timeout_seconds", 2)) * 1000
            )
            conn.execute(f"PRAGMA busy_timeout={busy_ms};")
            # Reasonable sync for tests
            conn.execute("PRAGMA synchronous=NORMAL;")
        except Exception:
            # PRAGMA failures should not break functionality
            pass
        return conn
    ...
    def create_jwt_token(
        ...
        - create_jwt_token(user=<User>)
        - create_jwt_token(user_id=..., username=..., email=..., role=...)
        """
        import uuid
        ...
        now = datetime.now(timezone.utc)
        # Compute expiry; if configured to 0 or negative, force immediate expiry
        if self.jwt_expiry_hours <= 0:
            # Set expiration in the past to guarantee immediate invalidation
        ...
        if user is not None:
            ...
            uname = user.username
            uemail = user.email
            urole_value = user.role.value if isinstance(user.role, UserRole) else str(user.role)
        else:
            if user_id is None or username is None or role is None:
                raise TypeError(
                    "create_jwt_token() requires either 'user' or user_id, username, and role"
                )
            uid = user_id
            uname = username
            uemail = email
            urole_value = role.value if isinstance(role, UserRole) else str(role)
    ...
    def verify_jwt_token(self, token: str) -> Optional[User]:
        ...
        try:
            # Decode token with leeway to tolerate small clock skew
            # If tokens are configured with immediate expiry (e.g., tests set jwt_expiry_hours=0),
            # use zero leeway to ensure strict expiration behavior.
            payload = jwt.decode(
                token,
                self.jwt_secret,
                algorithms=[self.jwt_algorithm],
                # Use strict expiration (no leeway) and enforce iat tolerance manually below
                leeway=0,
                options={
                    "require": ["exp", "iat"],
                    "verify_signature": True,
                    "verify_exp": True,
                    # Disable nbf verification to avoid false negatives in immediate-use tokens
                    "verify_nbf": False,
                    # Relax iat verification during decode; we'll validate it manually
                    "verify_iat": False,
                },
            )
            ...
            # Only require user_id; token_id is optional in contract tests.
            # If token_id is absent, skip DB-based revocation checks.
            if not user_id:
                return None

            # Check if token is revoked (only when token_id is present)
            if token_id:
                token_hash = hashlib.sha256(token.encode()).hexdigest()
                ...
                try:
                    ...
                    logger.debug(f"Skipping DB revocation check due to OperationalError: {oe}")

            # Construct user from JWT claims if not present in DB
            username = payload.get("username")
            email = payload.get("email")
            role_claim = payload.get("role")

            # Validate required claims (role is optional per contract expectations)
            if not all([user_id, username]):
                return None

            # Validate 'iat' is not set in the future (no positive skew beyond tolerance)
            try:
                iat_val = payload.get("iat")
                if iat_val is not None:
                    if isinstance(iat_val, (int, float)):
                        issued_at = datetime.fromtimestamp(float(iat_val), tz=timezone.utc)
                    else:
                        # Attempt ISO8601 parsing
                        issued_at = datetime.fromisoformat(str(iat_val).replace("Z", "+00:00"))
                        if issued_at.tzinfo is None:
                            issued_at = issued_at.replace(tzinfo=timezone.utc)
                    # Reject tokens only if iat is too far in the future beyond allowable leeway
                    now = datetime.now(timezone.utc)
                    if issued_at - now > timedelta(seconds=self.jwt_leeway_seconds):
                        return None
            except Exception:
                # If iat is malformed, consider token invalid
                return None

            # Map role claim to UserRole
            # Contract: if 'role' is missing entirely, default to VIEWER.
            # If 'role' key is present but invalid (e.g., None/empty/unknown), reject.
            ...
            # Return user constructed from claims
            return User(
                user_id=user_id,
                username=username,
    ...
    def verify_hmac_signature(
        ...
        Supports multiple signature formats and secret overrides:
        - "sha256=<hex>" format (e.g., X-Hub-Signature-256 headers)
        - Contract style: "t=<iso8601>,v1=<hex>" where timestamp is embedded
        - Raw hex string

        Args:
            payload: Payload to verify. Supports bytes, str (JSON), or dict. Strings are UTF-8 encoded.
            signature: Signature string in one of the supported formats.
            secret: Optional HMAC secret override (positional third arg). Defaults to self.webhook_secret.
            timestamp: Optional ISO8601 or epoch seconds, used for replay protection. If not provided,
                       it may be parsed from a contract-style signature string.
        """
        try:
            # Normalize and canonicalize payload
            canonical_payload_str: Optional[str] = None
            canonical_payload_bytes: bytes

            if isinstance(payload, (dict, list)):
                # Directly canonicalize JSON-able structures
                canonical_payload_str = json.dumps(
                    payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False
                )
                canonical_payload_bytes = canonical_payload_str.encode("utf-8")
            elif isinstance(payload, (bytes, bytearray)):
                # Attempt to parse JSON and canonicalize; otherwise use raw
                ...
                try:
                    ...
                    canonical_payload_str = payload_str
                    canonical_payload_bytes = payload
            elif isinstance(payload, str):
                # Attempt to parse JSON and canonicalize; otherwise use raw string
                payload_str = payload
                try:
                    obj = json.loads(payload_str)
                    canonical_payload_str = json.dumps(
                    ...
                    canonical_payload_str = payload_str
                    canonical_payload_bytes = payload_str.encode("utf-8")
            else:
            ...
            # Allow contract-style signature: "t=<ts>,v1=<sig>" and extract components
            parsed_timestamp: Optional[str] = None
            parsed_sig: Optional[str] = None

            if "," in signature and ("v1=" in signature or "t=" in signature):
                try:
                    parts = {}
                    for item in signature.split(","):
                        k, v = item.split("=", 1)
                        parts[k.strip()] = v.strip()
                    # Timestamp from signature if present
                    if "t" in parts:
                        parsed_timestamp = parts["t"]
                    # Prefer v1 signature
                    if "v1" in parts:
                        parsed_sig = parts["v1"]
                except Exception:
                    # Fall back to raw signature handling below if parsing fails
                    parsed_timestamp = None
                    parsed_sig = None

            # Use provided timestamp or parsed one
            if not timestamp and parsed_timestamp:
                timestamp = parsed_timestamp

            # Detect if payload embeds a timestamp field; if present, we will not enforce
            # external timestamp tolerance for default (sha256=<hex>) signature format
            has_embedded_timestamp = False
            try:
                if canonical_payload_str and canonical_payload_str.strip().startswith("{"):
                    obj = json.loads(canonical_payload_str)
                    if isinstance(obj, dict) and "timestamp" in obj:
                        has_embedded_timestamp = True
            except Exception:
                # Ignore JSON parsing issues for embedded timestamp detection
                has_embedded_timestamp = False

            # Check timestamp if provided (prevent replay attacks)
            # Apply tolerance when:
            # - contract-style signature is used (parsed_sig present), OR
            # - default signature is used AND payload does not embed its own timestamp
            if timestamp and (parsed_sig is not None or not has_embedded_timestamp):
                try:
                    # Normalize timestamp to aware UTC
                    ts = timestamp.strip()
                    request_time: datetime
                    # Accept UNIX epoch seconds (int/float) or ISO8601
                    if ts.replace(".", "", 1).isdigit():
                        # epoch seconds
                        request_time = datetime.fromtimestamp(float(ts), tz=timezone.utc)
                    else:
                        request_time = datetime.fromisoformat(ts.replace("Z", "+00:00"))
            ...
            # Choose secret to use (support third positional argument as secret)
            secret_to_use = (secret if secret is not None else self.webhook_secret).encode("utf-8")

            # Verify signature
            # Default expected signature: HMAC(secret, payload)
            expected_signature_default = hmac.new(
                secret_to_use, canonical_payload_bytes, hashlib.sha256
            ).hexdigest()

            # Contract-style expected signature: HMAC(secret, f"{timestamp}.{payload_str}")
            expected_signature_contract = None
            if timestamp:
                # Use ASCII-escaped canonical JSON to match signature generation in tests
                msg_payload = (
            ...
            if parsed_sig is not None:
                candidate_sig = parsed_sig
            if candidate_sig.startswith("sha256="):
                candidate_sig = candidate_sig[7:]

            # Prefer contract-style comparison when signature provided in contract format
            if parsed_sig is not None and expected_signature_contract is not None:
                is_valid = hmac.compare_digest(expected_signature_contract, candidate_sig)
            else:
                is_valid = hmac.compare_digest(expected_signature_default, candidate_sig)
