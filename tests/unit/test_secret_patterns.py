...
MODULE_PATH = os.path.join(REPO_ROOT, "scripts", "ops", "scan_secrets.py")
spec = importlib.util.spec_from_file_location("scan_secrets", MODULE_PATH)
scan = importlib.util.module_from_spec(spec)
spec.loader.exec_module(scan)  # type: ignore
PATTERNS = getattr(scan, "PATTERNS")
SAFE = getattr(scan, "SAFE_LINE_SUBSTRINGS")
...
def test_secret_patterns_runtime_strings(line, expected):
    ...
    for name, rx in PATTERNS:
        if rx.search(line) and not any(s in line for s in SAFE):
            found = name
            break
