...
class RobustDevelopment:
    ...
    def run_critical_tests(self):
        ...
        tests = [
            ...
            {
                "name": "UI初期化",
                "command": f"\"{py}\" -c \"import sys; sys.path.insert(0,'.'); from src.ui.modern_interface import ModernCursorAIInterface; import tkinter as tk; root=tk.Tk(); root.withdraw(); app=ModernCursorAIInterface(root); print('SUCCESS'); root.destroy()\"",
                "expected": "SUCCESS",
            },
            {
                "name": "AI機能",
                "command": f"\"{py}\" -c \"import sys; sys.path.insert(0,'.'); from src.core.kernel import Kernel; from src.core.memory import Memory; m=Memory(); k=Kernel(m); r=k.query_local_api('テスト'); print('AI_SUCCESS' if r.get('response_text') else 'AI_FAIL')\"",
                "expected": "AI_SUCCESS",
            },
        ]
        ...
        for test in tests:
            ...
            try:
                ...
                success = result.returncode == 0 and test["expected"] in result.stdout
