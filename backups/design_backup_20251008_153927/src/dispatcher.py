...
class TaskDispatcher:
    ...
    def acquire_lock(
        ...
        """Acquire a lock with priority and TTL"""
        now = datetime.utcnow()
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "DELETE FROM locks WHERE datetime(acquired_at, '+' || ttl || ' seconds') < datetime('now')"
            )
            ...
            try:
                conn.execute(
                    """
                    INSERT INTO locks (resource, owner, priority, ttl, acquired_at)
                    VALUES (?, ?, ?, ?, ?)
                """,
                    (resource, owner, priority.value, ttl, now.isoformat()),
                )
                conn.commit()
                ...
                # Lock exists, check priority
                cursor = conn.execute(
                    "SELECT owner, priority FROM locks WHERE resource = ?", (resource,)
                )
                result = cursor.fetchone()
                if result and priority.value > result[1]:
                    # Higher priority, take over
                    conn.execute(
                        """
                        UPDATE locks SET owner = ?, priority = ?, ttl = ?, acquired_at = ?
                        WHERE resource = ?
                    """,
                        (owner, priority.value, ttl, now.isoformat(), resource),
                    )
                    conn.commit()
                    logger.info(
                    ...
                    logger.warning(
                        f"Lock acquisition failed: {resource} held by {result[0] if result else 'unknown'}"
                    )
                    return False
    ...
    def dispatch_task(self, request: DispatchRequest) -> Dict[str, Any]:
