        ...
        try:
            ...
            payload = jwt.decode(
                ...
                options={
                    ...
                    "verify_nbf": False,
                    # Relax iat verification to improve time-skew tolerance while still requiring the claim
                    "verify_iat": False,
                },
            )
            ...
            if str(self.db_path) == ":memory:" and self._conn:
                ...
                row = cursor.fetchone()
                # Treat missing record as not revoked (allow initial validity)
                is_revoked = bool(row[0]) if row else False
                if is_revoked:
                    return None
            else:
                with self._connect() as conn:
