...
class OrchDashboard:
    ...
    def _agents_registry_path(self) -> Path:
        try:
            return Path(self.base_dir) / "data" / "agents_registry.json"
        except Exception:
    ...
    def _save_instructions(self, items: List[Dict[str, Any]]) -> None:
        ...
        def api_agents_register():
            try:
                data = request.get_json(silent=True) or {}
                agents = self._load_agents()
                agent_id = data.get("id") or f"AG-{int(time.time())}"
                # 重複排除
                agents = [a for a in agents if a.get("id") != agent_id]
                new_item = {
                    "id": agent_id,
                    "role": data.get("role") or "WORK",
                ...
                agents.append(new_item)
                self._save_agents(agents)
                return jsonify({"success": True, "agent": new_item})
            except Exception as e:
        ...
        def api_agents_update():
            try:
                        ...
                        a.update(
                            {
                                ...
                                "worktree": data.get("worktree", a.get("worktree")),
                                "capabilities": data.get("capabilities", a.get("capabilities")),
                                "status": data.get("status", a.get("status")),
                                "updated_at": datetime.now().isoformat(),
                            }
                        )
                        updated = a
                ...
                self._save_agents(agents)
                return jsonify({"success": True, "agent": updated})
            except Exception as e:
        ...
        def api_agents_report():
            try:
                ...
                try:
                    ...
                    pass
                return jsonify({"success": True, "report": found})
            except Exception as e:
