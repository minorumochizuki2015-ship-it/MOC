...
def main():
    root = Path(sys.argv[1]) if len(sys.argv)>1 else Path(r"C:\Users\User\Trae\Game_project")
    apk  = Path(sys.argv[2]) if len(sys.argv)>2 else (root/"Trae/Build/Android/TraeGame.apk")
    out_ui = root/"observability/ui/report"; out_ui.mkdir(parents=True, exist_ok=True)
    out_log = root/"data/logs/current"; out_log.mkdir(parents=True, exist_ok=True)
    out_mcp = root/"observability/mcp"; out_mcp.mkdir(parents=True, exist_ok=True)
    ...
    def _sha256(p: Path) -> str:
        try:
            ...
            return ""
    # メタデータDIR（未設定なら既定）
    os.environ.setdefault("ENHANCED_ANALYSIS_METADATA_DIR",
        str(root/"Trae/Build/Android/TraeGame_extracted/assets/bin/Data/Managed/Metadata"))
    # 事前検査
    if not apk.exists():
        ts = time.strftime("%Y%m%d_%H%M%S")
        err = {"error": "apk_not_found", "apk": str(apk), "ts": ts}
        if not is_ci:
            (out_log/f"analysis_error_{apk.stem}_{ts}.json").write_text(json.dumps(err, ensure_ascii=False, indent=2), encoding="utf-8")
        (out_mcp/f"analysis_error_{apk.stem}_{ts}.json").write_text(json.dumps(err, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    # 監査用にも複製
    if not is_ci:
        (out_log/f"analysis_{apk.stem}_{ts}.json").write_text(json.dumps(res, ensure_ascii=False, indent=2), encoding="utf-8")
