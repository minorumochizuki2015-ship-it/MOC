...
def create_checkpoint():
    """現在の状態をチェックポイントとして保存"""
    try:
        result = subprocess.run("git add -A && git commit -m \"checkpoint: 段階的検証\" --no-verify", 
...
def rollback_to_checkpoint():
    """最後のチェックポイントにロールバック"""
    try:
        result = subprocess.run("git reset --hard HEAD~1", shell=True, capture_output=True, text=True)
        if result.returncode == 0:
            print("🔄 ロールバック成功")
            return True
        else:
